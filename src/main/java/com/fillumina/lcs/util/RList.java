 ...
 */
public class RList<T> extends AbstractList<T> {
    ...
    private RList(List<T> list, int start, int end, boolean reverse) {
        this.list = list;
    ...
    public RList<T> subList(int fromIndex, int toIndex) {
        return new RList<>(list, start + fromIndex, start + toIndex, reverse);
    }
